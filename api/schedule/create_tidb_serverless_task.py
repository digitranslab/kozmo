...
def create_tidb_serverless_task():
    ...
    if not kozmo_config.CREATE_TIDB_SERVICE_JOB_ENABLED:
        return
    tidb_serverless_number = kozmo_config.TIDB_SERVERLESS_NUMBER
...
def create_clusters(batch_size):
    try:
        ...
        new_clusters = TidbService.batch_create_tidb_serverless_cluster(
            ...
            project_id=kozmo_config.TIDB_PROJECT_ID or "",
            api_url=kozmo_config.TIDB_API_URL or "",
            iam_url=kozmo_config.TIDB_IAM_API_URL or "",
            public_key=kozmo_config.TIDB_PUBLIC_KEY or "",
            private_key=kozmo_config.TIDB_PRIVATE_KEY or "",
            region=kozmo_config.TIDB_REGION or "",
        )
