...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        :return:
        """
        url = URL(str(kozmo_config.CODE_EXECUTION_ENDPOINT)) / "v1" / "sandbox" / "run"

        headers = {"X-Api-Key": kozmo_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=kozmo_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=kozmo_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=kozmo_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
