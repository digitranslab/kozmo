...
SSRF_DEFAULT_MAX_RETRIES = kozmo_config.SSRF_DEFAULT_MAX_RETRIES
...
def make_request(method, url, max_retries=SSRF_DEFAULT_MAX_RETRIES, **kwargs):
    ...
    if "timeout" not in kwargs:
        kwargs["timeout"] = httpx.Timeout(
            timeout=kozmo_config.SSRF_DEFAULT_TIME_OUT,
            connect=kozmo_config.SSRF_DEFAULT_CONNECT_TIME_OUT,
            read=kozmo_config.SSRF_DEFAULT_READ_TIME_OUT,
            write=kozmo_config.SSRF_DEFAULT_WRITE_TIME_OUT,
        )
    ...
    while retries <= max_retries:
        try:
            if kozmo_config.SSRF_PROXY_ALL_URL:
                with httpx.Client(proxy=kozmo_config.SSRF_PROXY_ALL_URL) as client:
                    response = client.request(method=method, url=url, **kwargs)
            elif kozmo_config.SSRF_PROXY_HTTP_URL and kozmo_config.SSRF_PROXY_HTTPS_URL:
                proxy_mounts = {
                    "http://": httpx.HTTPTransport(proxy=kozmo_config.SSRF_PROXY_HTTP_URL),
                    "https://": httpx.HTTPTransport(proxy=kozmo_config.SSRF_PROXY_HTTPS_URL),
                }
