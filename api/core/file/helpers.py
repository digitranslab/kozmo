...
def get_signed_file_url(upload_file_id: str) -> str:
    url = f"{kozmo_config.FILES_URL}/files/{upload_file_id}/file-preview"

    timestamp = str(int(time.time()))
    nonce = os.urandom(16).hex()
    key = kozmo_config.SECRET_KEY.encode()
...
def verify_image_signature(*, upload_file_id: str, timestamp: str, nonce: str, sign: str) -> bool:
    ...
    secret_key = kozmo_config.SECRET_KEY.encode()
    ...
    return current_time - int(timestamp) <= kozmo_config.FILES_ACCESS_TIMEOUT
