...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=kozmo_config.RELYT_HOST or "localhost",
                port=kozmo_config.RELYT_PORT,
                user=kozmo_config.RELYT_USER or "",
                password=kozmo_config.RELYT_PASSWORD or "",
                database=kozmo_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
