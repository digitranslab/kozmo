...
class LindormVectorStore(BaseVector):
    ...
    def create_collection(self, dimension: int, **kwargs):
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            method_name = kwargs.pop("method_name", kozmo_config.DEFAULT_INDEX_TYPE)
            space_type = kwargs.pop("space_type", kozmo_config.DEFAULT_DISTANCE_TYPE)
...
class LindormVectorStoreFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> LindormVectorStore:
        lindorm_config = LindormVectorStoreConfig(
            hosts=kozmo_config.LINDORM_URL or "",
            username=kozmo_config.LINDORM_USERNAME,
            password=kozmo_config.LINDORM_PASSWORD,
            using_ugc=kozmo_config.USING_UGC_INDEX,
        )
        using_ugc = kozmo_config.USING_UGC_INDEX
        ...
        if dataset.index_struct:
            ...
            index_type = kozmo_config.DEFAULT_INDEX_TYPE
            distance_type = kozmo_config.DEFAULT_DISTANCE_TYPE
