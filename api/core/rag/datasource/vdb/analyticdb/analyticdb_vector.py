...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if kozmo_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=kozmo_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=kozmo_config.ANALYTICDB_KEY_SECRET or "",
                region_id=kozmo_config.ANALYTICDB_REGION_ID or "",
                instance_id=kozmo_config.ANALYTICDB_INSTANCE_ID or "",
                account=kozmo_config.ANALYTICDB_ACCOUNT or "",
                account_password=kozmo_config.ANALYTICDB_PASSWORD or "",
                namespace=kozmo_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=kozmo_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=kozmo_config.ANALYTICDB_HOST,
                port=kozmo_config.ANALYTICDB_PORT,
                account=kozmo_config.ANALYTICDB_ACCOUNT or "",
                account_password=kozmo_config.ANALYTICDB_PASSWORD or "",
                min_connection=kozmo_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=kozmo_config.ANALYTICDB_MAX_CONNECTION,
                namespace=kozmo_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
