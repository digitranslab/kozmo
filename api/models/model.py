...
class KozmoSetup(db.Model):  # type: ignore[name-defined]
    __tablename__ = "kozmo_setups"
    __table_args__ = (db.PrimaryKeyConstraint("version", name="kozmo_setup_pkey"),)
...
class App(db.Model):  # type: ignore[name-defined]
    ...
    def api_base_url(self):
        return (kozmo_config.SERVICE_API_URL or request.host_url.rstrip("/")) + "/v1"
...
class Conversation(db.Model):  # type: ignore[name-defined]
    ...
    def inputs(self):
        ...
        for key, value in inputs.items():
            ...
            if isinstance(value, dict) and value.get("kozmo_model_identity") == FILE_MODEL_IDENTITY:
                ...
                isinstance(item, dict) and item.get("kozmo_model_identity") == FILE_MODEL_IDENTITY for item in value
            ):
                inputs[key] = []
                for item in value:
...
class Site(db.Model):  # type: ignore[name-defined]
    ...
    def app_base_url(self):
        return kozmo_config.APP_WEB_URL or request.url_root.rstrip("/")
