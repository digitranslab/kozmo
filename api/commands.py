...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = kozmo_config.VECTOR_STORE
...
def add_qdrant_doc_id_index(field: str):
    ...
    try:
        ...
        for binding in bindings:
            if kozmo_config.QDRANT_URL is None:
                raise ValueError("Qdrant URL is required.")
            qdrant_config = QdrantConfig(
                endpoint=kozmo_config.QDRANT_URL,
                api_key=kozmo_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=kozmo_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=kozmo_config.QDRANT_GRPC_PORT,
                prefer_grpc=kozmo_config.QDRANT_GRPC_ENABLED,
            )
            try:
