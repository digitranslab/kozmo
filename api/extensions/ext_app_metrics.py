...
def init_app(app: KozmoApp):
    ...
    def after_request(response):
        ...
        response.headers.add("X-Version", kozmo_config.CURRENT_VERSION)
        response.headers.add("X-Env", kozmo_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": kozmo_config.CURRENT_VERSION}),
            status=200,
