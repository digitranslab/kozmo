...
class AliyunOssStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = kozmo_config.ALIYUN_OSS_BUCKET_NAME
        self.folder = kozmo_config.ALIYUN_OSS_PATH
        oss_auth_method = aliyun_s3.Auth
        region = None
        if kozmo_config.ALIYUN_OSS_AUTH_VERSION == "v4":
            oss_auth_method = aliyun_s3.AuthV4
            region = kozmo_config.ALIYUN_OSS_REGION
        oss_auth = oss_auth_method(kozmo_config.ALIYUN_OSS_ACCESS_KEY, kozmo_config.ALIYUN_OSS_SECRET_KEY)
        self.client = aliyun_s3.Bucket(
            oss_auth,
            kozmo_config.ALIYUN_OSS_ENDPOINT,
            self.bucket_name,
            connect_timeout=30,
            region=region,
        )
