...
class AwsS3Storage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = kozmo_config.S3_BUCKET_NAME
        if kozmo_config.S3_USE_AWS_MANAGED_IAM:
            logger.info("Using AWS managed IAM role for S3")

            session = boto3.Session()
            region_name = kozmo_config.S3_REGION
            ...
            self.client = boto3.client(
                "s3",
                aws_secret_access_key=kozmo_config.S3_SECRET_KEY,
                aws_access_key_id=kozmo_config.S3_ACCESS_KEY,
                endpoint_url=kozmo_config.S3_ENDPOINT,
                region_name=kozmo_config.S3_REGION,
                config=Config(s3={"addressing_style": kozmo_config.S3_ADDRESS_STYLE}),
            )
        # create bucket
        try:
