...
from configs.app_config import KozmoConfig
...
def test_kozmo_config(example_env_file):
...
def test_flask_configs(example_env_file):
    ...
    # clear system environment variables
    os.environ.clear()
    flask_app.config.from_mapping(KozmoConfig(_env_file=example_env_file).model_dump())  # pyright: ignore
    ...
    assert config["SQLALCHEMY_DATABASE_URI"] == "postgresql://postgres:@localhost:5432/kozmo"
