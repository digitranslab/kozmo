...
A Kozmo App Service-API Client, using for build a webapp by request Service-API

## Usage

First, install `kozmo-client` python sdk package:

```
pip install kozmo-client
```
...
```python
from kozmo_client import CompletionClient
...
```python
import json
from kozmo_client import ChatClient
...
with open(file_path, "rb") as file:
    ...
    response = kozmo_client.file_upload("user_id", files)
